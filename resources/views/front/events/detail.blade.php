...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
                        ...
                        <div itemscope="" itemtype="http://data-vocabulary.org/Event">
                        	...
                        	<div class="top-info-wrap">
                        		<div class="event-img-left">
		                            <p>
		                                <img src="{{isset($event->logo) ? asset('logos/events/'.$event->logo) : '/front/images/event'. mt_rand(0,24). '.jpg'}}" width="150px" height="150px" title=" 2nd International Conference on Veterinary &amp; Animal Science" alt="{{$event->title}} &amp; {{isset($event->categories[0]) ? $event->categories[0]->name : ''}}" class="media-object">
		                            </p>
		                        </div>
                                ...
                                <h4 class="confo-title"><span itemprop="name"> {{$event->title}} &amp; {{isset($event->categories[0]) ? rtrim($allcats,",") : ''}}</span></h4>
                                <p><span>Date :</span>
                            ...
                            <br/>
                            <?php //dd($event->city); ?>
                            <span itemprop="summary">{{$event->title}} &amp; {{isset($event->categories[0]) ? rtrim($allcats,",") : ''}} will be held in <?= ($event->city != null)?$event->city->name:'' ?>,<?= ($event->city != null)? $event->city->state->country->name : ''?> on date {{$event->start_date}}</span>
            ...
            <div class="map-warp">
                <!-- <iframe width="1000" height="450" frameborder="0" style="border:0" src="https://www.google.com/maps/embed/v1/place?key=AIzaSyBQZfR7z56ZWcUre-EZ3m-9zS1sI797mko&amp;q={{$event->venue ?? $event->city->state->country->name ??$event->city->name}}"


                    >
                </iframe> -->
                <iframe height="450" width="1000" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q={{$event->venue ?? $event->city->state->country->name ??$event->city->name}}&output=embed&zoom=17" style="border:0;   width:100%"></iframe>
            </div>
