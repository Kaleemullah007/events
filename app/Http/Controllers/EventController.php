...
class EventController extends Controller
{
    ...
    public function changeStatus(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $user_email = Event::find($request->id);


// This code is used for Email sending
if(!empty($user_email->admin_email_address)){

 $data = array(
                        'success' => 'Event Status has been updated',
                        'id' => $request->id,
                        'name' => $user_email->title,
                    );
Mail::send(['text'=>'success'], $data, function($message) {
         $message->to($user_email->admin_email_address)->subject
            ('Event Registration');
         $message->from('info@way2conference.com','Way2conference');
      });
}
